...
var root_structured_text_node = []
var client = null;
var imageBlockId = null;

async function buildTree(nodes, is_root = false) {
    var structured_text_node = []
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        if (child.nodeName == "P") {
            ...
            if (child.textContent.trim() == "")
                for (let j = 0; j < res.length; j++) structured_text_node.push(res[j]);
            else
                structured_text_node.push({ type: 'paragraph', children: res });
            ...
            const mark = {
                SMALL: 'emphasis', I: 'emphasis', EM: 'emphasis',
            ...
            var uploadPhoto = await uploadToDatoCMS(child.src, client);
            if (uploadPhoto == null) {
            ...
            var bC = await buildTree(child.childNodes)
        ...
        var last_item = structured_text_node.length - 1;
        var non_root_blocks = ["span", "link"]
...
async function fetchRecords(type, field, value, client) {
    var records = await client.items.list({
        filter: {
            type: type,
